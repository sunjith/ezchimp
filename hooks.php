...
function ezchimp_hook_order($vars)
{
    ...
    $errors = $client = $subscriptions = $subscriptions1 = $subscriptions2 = $subscriptions3 = $sub = $sub_list = array();
    ...
    while ($module = mysql_fetch_assoc($result1)) {
        ...
        if ($product = mysql_fetch_assoc($result2)) {
            ...
            if ($productgroup = mysql_fetch_assoc($result3)) {
                ...
                $query = "SELECT DISTINCT `packageid` FROM `tblhosting` WHERE `userid`=$client_id";
                $result = mysql_query($query);
                $product_ids = array();
                while ($row = mysql_fetch_assoc($result)) {
                    $product_ids[] = $row['packageid'];
                }
                mysql_free_result($result);
                if ((!empty($groupings[$productgroup_name]))||(!empty($groupings1[$productgroup_name]))) {
                    if (!in_array($productgroup_name, $subscriptions)) {
                        $subscriptions[] = $productgroup_name;
                        $subscriptions1[] = $productgroup_name;
                        ...
                        $email = $client['email'];
                        if ($debug > 0) {
                            logActivity("ezchimp_hook_order: clientid - " . print_r($client_id, true));
                        }
                        if (!empty($client)) {
                            if ($debug > 0) {
                                logActivity("ezchimp_hook_order: sublist1 - " . print_r($sub, true));
                            }
                            foreach ($groupings[$productgroup_name] as $list_id1 => $list_groupings) {
                                if (!empty($list_groupings)) {
                                    ...
                                    if (!(is_array($list_groupings))) {
                                        if ($debug > 0) {
                                            logActivity("_ezchimp_get_client_subscriptions: empty main group - $subscriptions");
                                        }
                                        ...
                                        $field_id = $row['id'];
                                        if ($debug > 0) {
                                            logActivity("ezchimp_hook_order: subscription field3 - " . print_r($row['id'], true));
                                        }
                                        mysql_free_result($result);
                                        $query = "SELECT DISTINCT `relid` FROM `tblcustomfieldsvalues` WHERE `fieldid` = $field_id AND `value`='on'";
                                        $result = mysql_query($query);
                            ...
                            $sub2 = _ezchimp_get_listgroup_subscriptions($client_id);
                            if ($debug > 0) {
                                logActivity("ezchimp_hook_order: sublist2 - " . print_r($sub2, true));
                            }
                                foreach ($groupings[$productgroup_name] as $list_id1 => $list_groupings) {
                                    if (!empty($list_groupings)) {
                                        if ($debug > 0) {
                                        ...
                                        foreach ($list_groupings as $maingroups => $groups) {
                                            foreach ($sub2 as $subgrp => $subgrps) {
                            ...
                            $sub1 = _ezchimp_get_listgroup_subscriptions($client_id);
                            if ($debug > 0) {
                                logActivity("ezchimp_hook_order: sublist3 - " . print_r($sub1, true));
                            }
                            foreach($groupings1[$productgroup_name] as $list_id2 =>$list_unsub){
                               $subscription_groupings = array();
                               if (!empty($list_unsub)) {
                                    if ($debug > 0) {
                                        logActivity("ezchimp_hook_order: unsubscription1 group1 -$list_unsub ");
                                    }
                            ...
                            $sub3 = _ezchimp_get_listgroup_subscriptions($client_id);
                            if ($debug > 0) {
                                logActivity("ezchimp_hook_order: sublist4 - " . print_r($sub3, true));
                            }
                            foreach($groupings1[$productgroup_name] as $list_id2 =>$list_unsub){
                                $subscription_groupings = array();
                                if (!empty($list_unsub)) {
                                    ...
                                    foreach ($list_unsub as $maingroups => $groups) {
                                        $val='';
                                        foreach ($sub1 as $subgrp => $subgrps1) {
                                            foreach ($sub3 as $subgrp => $subgrps) {
                                                if (!($subgrps['unsubscribe'] == 1)) {
